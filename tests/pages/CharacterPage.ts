...
export class CharacterPage {
  ...
  async expectLoaded(nickname: string): Promise<this> {
    ...
    await expect(this.detailTablist, '캐릭터 상세 정보 탭리스트가 노출 되어야 한다').toBeVisible();
    await expect(this.infoTab, '기본 선택 탭은 캐릭터 정보탭이 선택되어야 한다').toHaveAttribute(
      'aria-selected',
      'true'
    );
    return this;
  ...
  async expectHeaderActionsVisible(nickname: string): Promise<this> {
    ...
    await expect(this.shareButton, '공유 버튼이 노출 되어야 한다').toBeVisible();
    await expect(this.shareButton, '공유 버튼이 활성화되어야 한다').toBeEnabled();
    await expect(this.favoriteButton, '즐겨찾기 추가 버튼이 노출 되어야 한다').toBeVisible();
    await expect(this.favoriteButton, '즐겨찾기 추가 버튼이 활성화되어야 한다').toBeEnabled();
    await expect(this.unionInfoLink, '유니온 정보 보기 링크가 노출 되어야 한다').toBeVisible();
  ...
  async openEquipmentTab(): Promise<this> {
    ...
    await expect(this.equipmentSearchBox, '장비 탭 클릭 후 장비 검색창이 노출 되어야 한다').toBeVisible();
    return this;
  }
  ...
  async openStatsTab(): Promise<this> {
    ...
    await expect(this.page.getByText('전투력', { exact: true }), '스탯 탭 클릭 후 전투력 항목이 노출 되어야 한다').toBeVisible();
    return this;
  }
  ...
  async expectInfoTabReady(nickname: string): Promise<this> {
    ...
    await expect(this.page.getByRole('heading', { name: '상세 정보' }), '상세 정보 heading이 노출 되어야 한다').toBeVisible();
    for (const label of ['월드', '직업', '성별', '길드', '데이터 기준일']) {
      await expect(this.page.getByText(label, { exact: true }), `${label} 항목이 노출 되어야 한다`).toBeVisible();
    }
    return this;
  }
  ...
  async expectEquipmentTabReady(): Promise<this> {
    // console.log('[CharacterPage] 장비 탭 내용 검증');
      await expect(this.equipmentSearchBox, '장비 검색창이 노출 되어야 한다').toBeVisible();
    await expect(this.page.getByRole('heading', { name: '칭호' }), '칭호 영역이 노출 되어야 한다').toBeVisible();
    await expect(this.page.getByRole('heading', { name: '장비' }), '장비 heading이 노출 되어야 한다').toBeVisible();
    await expect(this.optionDetailHeading, '옵션 상세 영역 heading이 노출 되어야 한다').toBeVisible();
    await expect(this.optionDetailPlaceholder, '옵션 상세 초기 안내 문구가 노출 되어야 한다').toBeVisible();

    for (const preset of ['현재 장착', '프리셋 1', '프리셋 2', '프리셋 3']) {
      await expect(
        this.page.getByRole('tab', { name: preset }),
        `${preset} 장비 프리셋 탭이 노출 되어야 한다`
      ).toBeVisible();
    }

    const cardCount = await this.equipmentCards.count();
    expect(cardCount, '장비 상세 보기 카드가 1개 이상 노출 되어야 한다').toBeGreaterThan(0);
    return this;
  }
  ...
  async openFirstEquipmentDetail(): Promise<this> {
    ...
    await expect(firstCard, '첫 번째 장비 카드가 노출 되어야 한다').toBeVisible();
  ...
  async expectStatsTabReady(): Promise<this> {
    ...
    await expect(this.page.getByText('전투력', { exact: true }), '전투력 항목이 노출 되어야 한다').toBeVisible();
    await expect(this.page.getByText('주스탯:'), '주스탯 항목이 노출 되어야 한다').toBeVisible();

    for (const groupName of ['공격 / 마력', '기본 스탯', '데미지', '크리티컬', '방어 / 기타', '이동', '포스']) {
      await expect(this.statGroup(groupName), `${groupName} 스탯 그룹이 노출 되어야 한다`).toBeVisible();
    }
    return this;
  }
